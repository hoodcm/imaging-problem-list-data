function iplApp() {
  return {
    ...
    computeFindingStatus(finding) {
      ...
      const sortedObs = [...finding.observations].sort((a, b) => new Date(b.exam_date) - new Date(a.exam_date));
    ...
    getGroupedObservations(finding) {
      ...
      const groupedArray = Object.values(groups).sort((a, b) => new Date(b.exam_date) - new Date(a.exam_date));
    ...
    get filteredFindings() {
      ...
      if (this.filterRegion !== 'all') {
        findings = findings.filter(
          (finding) =>
            finding.bodyRegions &&
            (finding.bodyRegions.includes(this.filterRegion) || finding.bodyRegions.includes('ALL')),
        );
      }
        ...
        const findingSections = [
          {
            title: 'Current',
            key: 'current',
            findings: examObservations.filter((f) => f.status === 'current'),
          },
          { title: 'Always', key: 'always', findings: examObservations.filter((f) => f.status === 'always') },
          {
            title: 'Resolved',
            key: 'resolved',
            findings: examObservations.filter((f) => f.status === 'resolved'),
          },
