...
| ~~PR-004~~ | ~~medium~~ | ~~De-duplicate review-callback wiring between `worker/extraction_jobs.py` and `extractor/runtime.py`.~~ Resolved (extracted `_build_review_callback()` helper in `runtime.py`). | `docs/extractor-agent-roadmap.md` |
...
| ~~PR-007~~ | ~~medium~~ | ~~Add/confirm inline orchestrator comments documenting findings/impression extraction gate semantics.~~ Resolved (inline comments added to orchestrator subpackage). | `docs/extractor-agent-roadmap.md` |
| ~~PR-008~~ | ~~medium~~ | ~~Unify logging style in touched runtime modules (`structlog` vs stdlib logging).~~ Resolved (all runtime modules now use structlog). | `docs/extractor-agent-roadmap.md`, `docs/logging-internals.md` |
| ~~PR-009~~ | ~~medium~~ | ~~Audit extractor UI status handling and ensure canonical stage/status-event contract only (no legacy-status assumptions).~~ Resolved (removed dead `apply_coding` label; remaining stages confirmed in use). | `docs/archive/ui-impact-runtime-unification.md` |
...
| ~~PR-011~~ | ~~low~~ | ~~Consolidate minor helper duplication: passthrough chunk helper reuse and review-callback wiring reuse.~~ Resolved (review-callback extracted in PR-004; passthrough chunk helper exists only once, no duplication). | `docs/extractor-agent-roadmap.md` |
